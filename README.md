...
管理耗材时，你需要知道正在使用哪一卷、半卷还剩多少、同一张资料卡先后对应过哪些卷。sohun 把耗材资料、入库批次、实物卷、当前装载和消耗记录分开保存，使一张可重复使用的 CUID/FUID 资料卡能够服务多卷耗材，同时保留每一卷自己的余量和历史。
...
检测到物理取下时，系统暂停该供料位，保留原卷余量与未结算任务。取消弹窗也不会把半卷清零。选择“维修暂取”后可装回原卷续用；普通取下或换料后，需要确认实际装入哪卷。只有明确选择“已用完”并再次确认，才执行耗尽结算。等待确认期间的云同步不会把临时取下误记成永久换卷。
...
可以。它是耗材资料卡，可以反复登记同类耗材。旧卷、半卷和新卷分别保存，卡片本身不代表某一卷只能使用一次。
