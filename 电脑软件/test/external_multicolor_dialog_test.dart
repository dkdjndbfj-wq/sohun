...
const _runGoldenTests = bool.fromEnvironment('SOHUN_RUN_GOLDENS');

Future<void> _expectGoldenIfEnabled(
  WidgetTester tester,
  Finder finder,
  String path,
) async {
  if (!_runGoldenTests) return;
...
void main() {
  ...
  testWidgets('color plan shows every sliced color and stock/create actions', (
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          ...
          personalRfidSpoolBindingsProvider.overrideWith((ref) => const {}),
    ...
    await _expectGoldenIfEnabled(
      tester,
      find.byType(ExternalMulticolorPlanDialog),
      'goldens/external_multicolor_plan_dialog.png',
    );

    await tester.tap(find.byKey(const ValueKey('choose-consumable-tool-0')));
    ...
    await _expectGoldenIfEnabled(
      tester,
      find.byKey(const ValueKey('external-consumable-picker')),
      'goldens/external_consumable_picker_dialog.png',
    );
  ...
  testWidgets(
    'logical colors may share stock and combined shortage is blocked',
    (tester) async {
      await tester.binding.setSurfaceSize(const Size(700, 720));
      addTearDown(() => tester.binding.setSurfaceSize(null));
      final lowStock = [
      ...
      final request = ExternalMulticolorPlanRequest(
        ...
        changePoints: const [
          FilamentChangePoint(layerNum: 5, toolIndex: 1, previousToolIndex: 0),
        ],
        createdAt: now,
      );
      ...
      for (final tool in [0, 1]) {
        await tester.tap(find.byKey(ValueKey('choose-consumable-tool-$tool')));
        await tester.pumpAndSettle();
        await tester.tap(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      const data = FilamentChangeReminderData(
        ...
        changeIndex: 2,
        changeCount: 4,
        upcoming: [FilamentChangePoint(layerNum: 26, toolIndex: 0)],
      ...
      await _expectGoldenIfEnabled(
        ...
        'goldens/filament_change_reminder_dialog.png',
      );
    },
  );
